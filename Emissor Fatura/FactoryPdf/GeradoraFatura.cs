...
{
    public static class GeradoraFatura
    {
        public static void GerarFatura(Fatura f)
        {
            var caminho = Constante.PATH_FATURAS;
            var nomeArquivo = string.Concat(caminho, f.Cliente.Nome.Replace(" ","_"), "_", f.Numero, ".pdf");
            ...
            using (var doc = new PdfSharp.Pdf.PdfDocument())
            {
                ...
                grafico.DrawString(string.Concat(Constantes.CAMPO_CNPJ, f.Emissor.Cnpj), new XFont("Consolas", 10, XFontStyle.Bold), corFonte, 180, 60,XStringFormats.Center);
                grafico.DrawString(string.Join(", ",f.Emissor.Endereco,f.Emissor.EnderecoNumero,f.Emissor.EnderecoBairro,f.Emissor.EnderecoMunicipio,f.Emissor.Uf), new XFont("Consolas", 8, XFontStyle.BoldItalic), corFonte, 180, 75,XStringFormats.Center);
                grafico.DrawString(string.Concat(Constantes.CAMPO_CEP,f.Emissor.EnderecoCep," ",Constantes.CAMPO_FONE, f.Emissor.Fone), new XFont("Consolas", 8, XFontStyle.BoldItalic), corFonte, 180, 85,XStringFormats.Center);


                //Dados da Fatura
                int alinhamento = 360;
                grafico.DrawString(Constantes.DESC_MODELO, new XFont("Consolas", 10, XFontStyle.Bold), corFonte, 390, 12); 
                grafico.DrawString(Constantes.CAMPO_EMISSAO, new XFont("Consolas", 10, XFontStyle.Bold), corFonte, alinhamento, 30);
                grafico.DrawString(f.DataEmissao.ToString("dd/MM/yyyy"), new XFont("Consolas", 10, XFontStyle.Regular), corFonte, 440, 30);
                 
                grafico.DrawString(Constantes.CAMPO_INSC_M, new XFont("Consolas", 10, XFontStyle.Bold), corFonte, alinhamento, 50);
                grafico.DrawString(f.Emissor.InscMunicipal, new XFont("Consolas", 10, XFontStyle.Regular), corFonte, 470, 50);

                grafico.DrawString(Constantes.CAMPO_NAT_OP, new XFont("Consolas", 10, XFontStyle.Bold), corFonte, alinhamento, 60);
                grafico.DrawString(Constantes.NAT_OP, new XFont("Consolas", 10, XFontStyle.Regular), corFonte, 480, 60);

                grafico.DrawString(Constantes.SERIE, new XFont("Consolas", 10, XFontStyle.Bold), corFonte, alinhamento, 70);
                grafico.DrawString(Constantes.SERIE_TIPO, new XFont("Consolas", 10, XFontStyle.Regular), corFonte, 400, 70);

                grafico.DrawString(string.Concat("Nº: ", f.Numero.ToString("00000")), new XFont("Consolas", 15, XFontStyle.Bold), corFonte, 440, 90);

                //Dados do cliente
                alinhamento = 10;
                grafico.DrawString(Constantes.CAMPO_NOME_CLIENTE, new XFont("Consolas", 12, XFontStyle.Bold), corFonte, alinhamento, 120);
                grafico.DrawString(f.Cliente.Nome, new XFont("Consolas", 12, XFontStyle.Regular), corFonte, 120, 120);

                grafico.DrawString(Constantes.CAMPO_ENDERECO, new XFont("Consolas", 12, XFontStyle.Bold), corFonte, alinhamento, 135);
                grafico.DrawString(string.Concat(f.Cliente.Endereco, ", ", f.Cliente.EnderecoNumero), new XFont("Consolas", 12, XFontStyle.Regular), corFonte, 80, 135);

                grafico.DrawString(Constantes.CAMPO_MUNICIPIO, new XFont("Consolas", 12, XFontStyle.Bold), corFonte, alinhamento, 150);
                grafico.DrawString(f.Cliente.EnderecoMunicipio, new XFont("Consolas", 12, XFontStyle.Regular), corFonte, 100, 150);

                grafico.DrawString(Constantes.CAMPO_ESTADO, new XFont("Consolas", 12, XFontStyle.Bold), corFonte, 300, 150);
                grafico.DrawString(f.Cliente.Uf, new XFont("Consolas", 12, XFontStyle.Regular), corFonte, 350, 150);

                grafico.DrawString(Constantes.CAMPO_CEP, new XFont("Consolas", 12, XFontStyle.Bold), corFonte, 400, 150);
                grafico.DrawString(f.Cliente.EnderecoCep, new XFont("Consolas", 12, XFontStyle.Regular), corFonte, 440, 150);

                grafico.DrawString(Constantes.CAMPO_CNPJ, new XFont("Consolas", 12, XFontStyle.Bold), corFonte, alinhamento, 165);
                grafico.DrawString(f.Cliente.Cnpj, new XFont("Consolas", 12, XFontStyle.Regular), corFonte, 50, 165);

                grafico.DrawString(Constantes.CAMPO_INSC_M, new XFont("Consolas", 12, XFontStyle.Bold), corFonte, 250, 165);
                grafico.DrawString(f.Cliente.InscMunicipal, new XFont("Consolas", 12, XFontStyle.Regular), corFonte, 380, 165);

                //Valor por extenso
                grafico.DrawString(Constantes.CAMPO_VALOR_EXTENSO, new XFont("Consolas", 12, XFontStyle.Bold), corFonte, alinhamento, 190);
                ...
                //                                      Constantes.NOME_EMPRESA), new XFont("Consolas", 12), XBrushes.Black, new XRect(5, 250, pagina.Width - 5, pagina.Height));
                ...
                grafico.DrawString(Constantes.CAMPO_PER_SERV, new XFont("Consolas", 10, XFontStyle.Bold),corFonte,alinhamento,490);
                grafico.DrawString(f.TempoPrestacao, new XFont("Consolas", 10, XFontStyle.Regular),corFonte,120,490);

                grafico.DrawString(Constantes.DADOS_BANCO, new XFont("Consolas", 10, XFontStyle.Bold), corFonte, alinhamento, 510);

                grafico.DrawString(Constantes.CAMPO_DADOS_BANCO_BC, new XFont("Consolas", 10, XFontStyle.Bold), corFonte, alinhamento, 520);
                grafico.DrawString("Banco do Brasil", new XFont("Consolas", 10, XFontStyle.Regular), corFonte, 50, 520);

                grafico.DrawString(Constantes.CAMPO_DADOS_BANCO_AG, new XFont("Consolas", 10, XFontStyle.Bold), corFonte, alinhamento, 530);
                grafico.DrawString("3344-5", new XFont("Consolas", 10, XFontStyle.Regular), corFonte, 60, 530);

                grafico.DrawString(Constantes.CAMPO_DADOS_BANCO_CC, new XFont("Consolas", 10, XFontStyle.Bold), corFonte, alinhamento, 540);
                grafico.DrawString("44.8888-2", new XFont("Consolas", 10, XFontStyle.Regular), corFonte, 95, 540);

                grafico.DrawString(Constantes.CAMPO_EMPENHO, new XFont("Consolas", 10, XFontStyle.Bold), corFonte, alinhamento, 555);
                grafico.DrawString(f.Empenho, new XFont("Consolas", 10, XFontStyle.Regular), corFonte, 95, 555);

                grafico.DrawString(Constantes.CAMPO_OBS, new XFont("Consolas", 10, XFontStyle.Bold), corFonte, 195, 490);
                formatacao.DrawString(f.Observacao, new XFont("Consolas", 12), XBrushes.Black, new XRect(195, 500, pagina.Width - 200, pagina.Height));

                //Legislacao e valor
                grafico.DrawString(Constantes.LEGIS_1, new XFont("Consolas", 15, XFontStyle.Bold), corFonte, 210, 580,XStringFormats.Center);
                formatacao.DrawString(Constantes.LEGIS_2, new XFont("Consolas", 12,XFontStyle.BoldItalic), XBrushes.Black, new XRect(alinhamento, 600, pagina.Width *(0.8), pagina.Height));

                grafico.DrawString(Constantes.CAMPO_VALOR, new XFont("Consolas", 15, XFontStyle.Bold), corFonte, 530, 580, XStringFormats.Center);
        ...
        private static void Deleta(string caminho)
        {
            if (Existe(caminho))
            {
                File.Delete(caminho);
